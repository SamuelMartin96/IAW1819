...
<%@page import="es.cj.dao.ConcesionarioDAOImpl"%>
<%@page import="es.cj.bean.Concesionario"%>
<%@page import="java.util.List"%>
<%@page import="es.cj.dao.ConcesionarioDAO"%>
		...
		<table class="table table-striped table-white table-hover">
			...
			<%
				if (session.getAttribute("usuarioWeb") == null || session.isNew()) {
					...
					ConcesionarioDAO cDAO = new ConcesionarioDAOImpl();
					List<Concesionario> concesionarios = cDAO.listar(con, (Usuario) session.getAttribute("usuarioWeb"));
			%>
			<div class="row cold-md-12 text-center">
				...
				%>
				<tr>
					<td><%=c.getIDConcesionario()%></td>
					<td><%=c.getNombre()%></td>
					<td><%=c.getFechaApertura()%></td>
					<td><%=c.getCantidad()%></td>
					<td><%=c.getIDVehiculo()%></td>
					</td>
				</tr>
				<%
					}
